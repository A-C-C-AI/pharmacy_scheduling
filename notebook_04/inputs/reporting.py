"""The human-readable report is generated from the same verified evidence as notebook 05."""
import pandas as pd
...
def table(frame):
    ...
    return '\n'.join(['| '+' | '.join(map(text,frame.columns))+' |',
                      '| '+' | '.join(['---']*len(frame.columns))+' |']+
                     ['| '+' | '.join(map(text,row))+' |' for row in frame.itertuples(index=False,name=None)])


def build_report(h,o,scenarios,changes,robustness,spec,profile,checklist):
    r=robustness['results'];N=profile['rotating_pharmacies'];p=spec['parameters']
    comp=scenarios[['scenario','assignments','total_range','h24_range','pareto_efficient','recommended']].copy()
    comp.columns=['Scenario','Assignments','Total range','H24 range','Nondominated*','Selected']
    groups=pd.DataFrame(robustness['group_summary'])[['rotation_group','group_size','scheduled_active_absences','recoverable_within_group','historical_candidates_requiring_approval','manual_without_candidate']]
    groups.columns=['Group','Size','Scheduled cases','Within group','Candidate / approval','No candidate']
    metrics=pd.DataFrame([
        ['Assignments',h['assignments'],o['assignments']],
        ['Total workload range',h['total_range'],o['total_range']],
        ['H24 workload range',h['h24_range'],o['h24_range']],
        ['Total Jain',f"{h['total_jain_index']:.9f}",f"{o['total_jain_index']:.9f}"],
        ['Total Gini',f"{h['total_gini']:.9f}",f"{o['total_gini']:.9f}"],
        ['Total population variance',f"{h['total_population_variance']:.9f}",f"{o['total_population_variance']:.9f}"],
        ['H24 Jain',f"{h['h24_jain_index']:.9f}",f"{o['h24_jain_index']:.9f}"],
        ['H24 Gini',f"{h['h24_gini']:.9f}",f"{o['h24_gini']:.9f}"],
    ],columns=['Measure','Historical','Fairness-aware'])
    active_changes=int(changes.active_set_changed.sum());h24_changes=int(changes.h24_set_changed.sum())
    h24_only=int((~changes.active_set_changed & changes.h24_set_changed).sum())
    automatic=r['recoverable_within_group'];scheduled=r['scheduled_active_absences'];manual=r['total_requiring_human_intervention']
    candidate=r['historical_candidates_requiring_approval'];blank=r['manual_cases_without_candidate']
    report = f'''# Fair and Constraint-Aware Pharmacy Shift Scheduling

## Abstract

This project investigates pharmacy duty scheduling as a constrained allocation of workload. The sole empirical input is the supplied TURNI CSV. The Bologna city-area calendar contains {profile['weeks']} weekly blocks, {N} rotating pharmacies and {h['assignments']} assignments, organized into 42 recurring patterns and seven dominant rotation groups. A mixed-integer linear program preserves a historical staffing floor, H24 duty count, primary-group eligibility, a horizon workload ceiling and a policy of balanced H24 cycle counts. It compares efficiency-first, fairness-aware and full-group scenarios using lexicographic priorities.

The selected fairness-aware schedule has {o['assignments']} assignments, total workload range {o['total_range']} and H24 range {o['h24_range']}, compared with {h['assignments']}, {h['total_range']} and {h['h24_range']} historically. Total Jain improves slightly, but total Gini increases slightly; fairness improvement is metric-specific. {len(changes)} complete nT patterns change, including {h24_only} with H24-only changes.

The robustness experiment tests {r['tested_pattern_absences']} eligible pharmacy-pattern cases, of which {scheduled} affect scheduled pharmacies. {automatic} ({100*automatic/scheduled:.1f}%) have a materialized, model-feasible same-group response. All {manual} remaining cases ({100*manual/scheduled:.1f}%) require human intervention. {candidate} have a historically evidenced candidate; these are not automatically authorized. These are completed results of a dataset-only computational case study; unrecorded real-world variables are excluded by design.
...
| Property | Value |
| --- | --- |
| Weekly blocks | {profile['weeks']} |
| Distinct nT patterns | 42 |
| Rotating pharmacies | {N} |
| Historical assignment rows | {h['assignments']} |
| Historical H24 assignments | {h['h24_total']} |
| Historical STANDARD assignments | {h['assignments']-h['h24_total']} |
| Weekly historical staffing | 17-18 |
| H24 pharmacies in each block | 3 |
| Maximum assignments per pharmacy in the horizon | 8 |
| Excluded non-rotation facilities | {profile['non_rotation_facilities']} |
...
The canonicalizer reconstructs dates, nT labels, groups, rounds, shift types and access modes. It verifies row counts, unique pharmacy-week assignments, consecutive weekly starts, adjacent boundaries, nT progression and consistent week metadata. The baseline checks repeated patterns and the constant H24 count. Identical nT values reuse the same pharmacy, location, shift and access signature across their calendar occurrences; there are {h['repeated_matches']} matching repeated-pattern comparisons out of {h['repeated_pairs']}.

For candidate periods 2 through 41, membership purity is the sum, across pharmacies, of the largest assignment count in any nT residue class, divided by all assignment rows. The leading period is {profile['group_period_candidate']} with purity {profile['group_membership_purity']:.6f}. One historical cross-group row means purity is not perfect. Dominant historical membership determines each pharmacy's primary modeled group; this is an inference from observations, not an externally supplied authorization list.

The revised permutation sensitivity test randomizes the 42 nT labels as intact pattern blocks, preserving each pattern's pharmacy membership and repeated-week multiplicity. Each randomization repeats the entire candidate-period search. It uses B={profile['group_period_permutations_run']} permutations and finds b={profile['group_period_permutation_exceedances']} null maxima at least as extreme as the observed maximum. The corrected Monte Carlo p-value is (b+1)/(B+1) = {profile['group_period_permutation_p_value']:.6f}. Randomly sampled permutation p-values should not be reported as zero. [2]
...
Jain's index is J(w) = (sum w_i)^2 / (N * sum w_i^2). For a nonzero nonnegative allocation its range is 1/N to 1, with 1 representing equality. [3] The project computes it descriptively; it is not a MILP objective. Gini is G(w) = sum over i,j of |w_i-w_j| / (2*N*sum w_i). Higher Gini indicates more inequality. Both metrics are evaluated across all {N} pharmacies, including any zero H24 counts. The code's all-zero convention is Jain=1 and Gini undefined; no reported schedule has zero aggregate workload. For a finite nonnegative N-pharmacy allocation, the maximum uncorrected Gini is (N-1)/N; no finite-sample correction is applied.
...
The fairness-aware schedule removes {h['assignments']-o['assignments']} assignments ({100*(h['assignments']-o['assignments'])/h['assignments']:.2f}% of history) and uses 22 more assignments than efficiency-first. It narrows total workload from 6-8 to 7-8 assignments and H24 workload from 0-3 to 1-2 assignments per pharmacy. Full-group matches those ranges but uses eight more assignments. It is dominated under the selected three metrics, yet has better total Jain and Gini than fairness-aware. Thus the preferred scenario depends on the specific fairness objective, not on a universal ordering of fairness.

{table(metrics)}

Total Jain increases slightly. Total Gini increases from {h['total_gini']:.9f} to {o['total_gini']:.9f}, a small worsening of {o['total_gini']-h['total_gini']:.9f}; both round to 0.0333 at four decimals. It would be incorrect to say that all fairness indices improve. The strongest positive results are the reduced total and H24 ranges and the improved H24 distribution.

There are {len(changes)} changed full patterns and {42-len(changes)} unchanged patterns in this executed solution. {active_changes} patterns change active membership and {h24_changes} change H24 membership; {h24_only} have H24-only changes. nt_change_explanation.csv records both types, including added and removed H24 IDs. A count based only on active pharmacies misses the H24-only changes. Patterns should not be called overstaffed merely because they exceed the historical-minimum proxy.
...
The experiment considers {r['tested_pattern_absences']} eligible pharmacy-pattern pairs: each of {N} pharmacies has six primary-group patterns. {r['no_change_required']} cases involve an inactive pharmacy and require no response; they are excluded from the recovery-rate denominator. The {scheduled} scheduled cases are evaluated one at a time from the same base schedule, not cumulatively.
...
The same-group modeled recovery rate is {automatic}/{scheduled} = {100*automatic/scheduled:.1f}%. All {manual} remaining cases ({100*manual/scheduled:.1f}%) require human intervention. Of these, {candidate} have a historical candidate and {blank} have no pre-filled candidate. The authorization template includes all {manual}, with approval fields left blank.

The observed exception is P0076 at nT=21 in group 7, despite a dominant group of 5. The source documents the occurrence, not its administrative reason, current availability, or authorization to replace every absent group-7 member. For the {candidate} corresponding cases, the candidate is added as STANDARD; when H24 is lost, the group's H24 re-optimization supplies it. {r['historical_candidates_passing_modeled_constraints']} of these candidate schedules pass all represented constraints using the exact historical eligibility exception. This modeled check still cannot verify legal eligibility, availability, geography or authority approval. They are never counted as automatic recoveries. Within this academic project, leaving these approval fields blank is the completed representation of that boundary; filling them with invented approvals would be incorrect.

All {r['H24_reoptimizations_feasible']} of the {r['H24_reoptimizations_tested']} attempted H24 recovery models are feasible. Some belong to cases where staffing still fails. H24-only feasibility is therefore not full schedule feasibility, and this result does not establish that H24 will never be a bottleneck in other absence patterns or operational settings.
...
{table(checklist.rename(columns={'project_component':'Component','evidence':'Evidence','status':'Status'}))}
...
The optimization formulation is appropriate for the supplied pharmacy-rotation data and the stated count-based fairness objective. It preserves the modeled historical rotation structure and staffing/H24 constraints while reducing total assignments from {h['assignments']} to {o['assignments']}, total range from {h['total_range']} to {o['total_range']} and H24 range from {h['h24_range']} to {o['h24_range']}.

The improvement is precise rather than universal: total Jain improves slightly, total Gini worsens slightly, and full-group is better on those two descriptive total-count indices while using more assignments. There are {len(changes)} changed complete patterns, including {h24_only} H24-only changes. The experiment finds {automatic} within-group recoveries among {scheduled} scheduled pattern cases and {manual} cases requiring a cross-group decision; {candidate} have a historically supported, model-feasible candidate without an invented approval.
...
'''
    ...
    raw=pd.read_csv(Path("output/canonical_schedule.csv"))
    selected=pd.read_csv(Path("output/optimized_schedule_fairness_aware.csv"))
    plan=pd.read_csv(Path("output/single_absence_response_plan.csv"),keep_default_na=False)
    all_scenarios=scenarios.set_index("scenario").loc[["Historical","Efficiency-first","Fairness-aware","Full-group"]].reset_index()
    count_table=all_scenarios[["scenario","total_min","total_max","h24_min","h24_max","total_jain","total_gini","h24_jain","h24_gini"]].copy()
    for c in ["total_jain","total_gini","h24_jain","h24_gini"]:count_table[c]=count_table[c].map(lambda x:f"{x:.6f}")
    count_table.columns=["Scenario","Total min","Total max","H24 min","H24 max","Total Jain","Total Gini","H24 Jain","H24 Gini"]
    stability=all_scenarios[["scenario","active_calendar_decision_changes","h24_calendar_decision_changes"]].copy()
    stability.columns=["Scenario","Active decision changes","H24 decision changes"]
    distributions=[]
    for label,frame in [("Historical",raw),("Fairness-aware",selected)]:
        for burden in ["Total","H24"]:
            rows=frame if burden=="Total" else frame.loc[frame.shift_type.eq("H24")]
            counts=rows.groupby("pharmacy_id").size().reindex(sorted(raw.pharmacy_id.unique()),fill_value=0)
            for workload,n in counts.value_counts().sort_index().items():distributions.append([label,burden,int(workload),int(n)])
    dist=pd.DataFrame(distributions,columns=["Schedule","Workload type","Assignments per pharmacy","Pharmacies"])
    counts=raw.groupby("pharmacy_id").access_mode.first().value_counts()
    report=report.replace("## 3. Computational pipeline",f"### Access modes, locations and source fidelity\n\nThe reconstructed calendar has {raw.location_id.nunique()} distinct pharmacy-location identifiers for {raw.pharmacy_id.nunique()} pharmacies. {int(counts.get('P',0))} pharmacies have mode P and {int(counts.get('C',0))} have mode C; no pharmacy changes P/C label across its observed rows. One pharmacy has two observed locations. Locations are retained as identifiers and are not converted into a geographic coverage model. Every accepted row preserves its source-row reference, and all accepted rows occur exactly once in the canonical output.\n\n## 3. Computational pipeline")
    report=report.replace("## 4. Fairness definition",f"### Numerical settings for the exploratory test\n\nCandidate periods are {profile['candidate_period_min']} through {profile['candidate_period_max']}, inclusive. The random seed is {profile['permutation_random_seed']}. The mean of the null best-purity values is {profile['null_best_purity_mean']:.6f}, and their maximum is {profile['null_best_purity_max']:.6f}. The number of permutations controls Monte Carlo resolution; it is a computational precision setting, not a pharmacy-data value or a fairness threshold. A 0.01 significance cutoff is an explicit screening choice used to stop the pipeline if this exploratory structural evidence is weak, rather than a requirement inferred from pharmacy data. The complete search is repeated for each draw, so the null comparison accounts for selection of the leading candidate.\n\n## 4. Fairness definition")
    report=report.replace("## 7. Structural limits", "### All scenario fairness statistics\n\n"+table(count_table)+"\n\n### Workload distributions\n\n"+table(dist)+"\n\nThese distributions make the range and inequality calculations auditable from counts of pharmacies rather than only aggregate indices. All N pharmacies enter each distribution; zeros are included for historical H24.\n\n### Changes measured at calendar level\n\n"+table(stability)+"\n\nA changed active or H24 decision means an added or removed pharmacy-week binary decision. Replacing one pharmacy with another counts as two changes, and recurring nT patterns contribute once for each affected calendar occurrence. These counts therefore differ from the number of distinct changed patterns.\n\n## 7. Structural limits")
    readable=changes[["nT","historical_count","optimized_count","added","removed","h24_added","h24_removed"]].copy()
    for c in ["added","removed","h24_added","h24_removed"]:readable[c]=readable[c].map(lambda x:', '.join(json.loads(x)) or '-').astype(str)
    readable.columns=["nT","Before","After","Active added","Active removed","H24 added","H24 removed"]
    appendices=build_appendices(raw,selected,plan,spec,readable)
    report=report.replace("## References",appendices+"\n\n## References")
    return report


def build_appendices(raw,selected,plan,spec,changes):
    policy_frame=pd.DataFrame([{"Policy":k.replace('_',' '),"Justification":v} for k,v in spec["policy_justifications"].items()])
    status=plan.loc[plan.was_scheduled_active].groupby("response_status").size().reset_index(name="Cases")
    status.columns=["Response category","Cases"]
    status["Response category"]=status["Response category"].str.replace('_',' ').str.lower()
    example=plan.loc[plan.response_status.eq("SAME_GROUP_ACTIVE_REPLACEMENT_AND_H24_REOPTIMIZATION")].sort_values(["nT","unavailable_pharmacy_id"]).iloc[0]
    return f"""## Appendix A. Full derivation and decision trace
...
Consider the simulated absence of {example['unavailable_pharmacy_id']} at nT={int(example['nT'])}, group {int(example['rotation_group'])}. This pharmacy is active and H24 in the selected schedule. The same-group active replacement is {example['same_group_active_replacement_id']}. The saved H24 changes are: {example['H24_change_details']}. The complete revised schedule passes staffing, H24, horizon workload, cycle-balance, absence, eligibility and repeated-pattern checks. This is an illustrative row selected from the exported response plan, not an invented event or an observation of an actual absence.
